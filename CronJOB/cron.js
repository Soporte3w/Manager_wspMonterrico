 ...
 * 2. Si la API devuelve registros, **borramos el histórico `sentMessages`**, 
 *    validamos cada mensaje, descartamos duplicados o ya enviados, y
 *    actualizamos `messageQueue` acorde.
...
async function fetchMessageQueue() {
  try {
    ...
    // ******************************************************************
    // NUEVA LÓGICA: Si la API devuelve registros, reiniciamos el archivo
    // de mensajes enviados, de modo que no queden IDs "pegados".
    // ******************************************************************
    sentMessages.clear();
    ...
    for (const msg of incomingMessages) {
      ...
      // Descartar si está en proceso (aunque acabamos de vaciar sentMessages)
      if (inProgressMessages.has(value.idSendmessage)) {
        logger.debug(`Mensaje ${value.idSendmessage} ignorado: ya en progreso`);
        continue;
      }
      // Descartar si está en sentMessages (normalmente vacío tras limpiar, salvo que se haya enviado en esta misma ejecución)
...
async function sendMessage(instance, messageData, attempt = 1) {
  ...
  try {
    ...
    } else if (["imagen", "video", "pdf"].includes(messageData.tipo)) {
      requestBody = {
        number: messageData.tenvio,
        caption: `*${messageData.titulo}*\n\n${messageData.mensaje}`,
        media: messageData.media,
        fileName:
          messageData.tipo === "pdf"
            ? `${messageData.titulo}.pdf`
            : messageData.tipo === "imagen"
            ? "img.jpg"
            : "video.mp4",
      };
