...
const API_BASE_URL = "https://wsp.monterrico.app";
const EXTERNAL_API_BASE_URL = "http://188.245.38.255:5000"; // URL base de la nueva API
const API_KEY = "e6c371b25066e172244a6548361e9689";
...
app.get("/api/instances", async (req, res) => {
  try {
    ...
    // Filtrar instancias cuyo nombre comienza con "MASIVO"
    const filteredInstances = response.data.filter(instance =>
      instance.name && instance.name.startsWith('MASIVO')
    );



    res.status(response.status).json(filteredInstances);
  } catch (error) {
